...
export function buildMockFamily(): Omit<FamilyTree, "collapsed"> {
  ...
  const date = (year: number) =>
    `${year}-${pad(randInt(1, 12))}-${pad(randInt(1, 28))}`;
  ...
  const founderH = makePerson(
    "male",
    foundingSurname,
    BASE_YEAR + randInt(-2, 1),
  );
  const founderW = makePerson(
    "female",
    pick(SURNAMES),
    BASE_YEAR + randInt(0, 4),
  );
  const c0 = makeCouple(founderH, founderW, BASE_YEAR + 24);

  let frontier: CoupleSeed[] = [
    { coupleId: c0, childSurname: foundingSurname },
  ];
  ...
  for (let gen = 1; gen < GENERATIONS && frontier.length > 0; gen++) {
    ...
    frontier.forEach((parent, parentIdx) => {
      ...
      for (let k = 0; k < kids; k++) {
        ...
        const childId = makePerson(
          gender,
          parent.childSurname,
          birthYear + randInt(-4, 4),
        );
      ...
      children.forEach((child, childIdx) => {
        ...
        const spouseId = makePerson(
          spouseGender,
          pick(SURNAMES),
          birthYear + randInt(-4, 4),
        );
        const coupleId = makeCouple(
          child.id,
          spouseId,
          birthYear + randInt(22, 30),
        );
